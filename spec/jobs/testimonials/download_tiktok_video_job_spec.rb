...
RSpec.describe Testimonials::DownloadTiktokVideoJob do
  ...
  it "attaches the downloaded video and enqueues GenerateQuoteTextJob on success" do
    testimonial = make_tiktok_testimonial
    ...
    expect(enqueued_ids).to eq([ testimonial.id ])
    testimonial.reload
  ...
  it "does not attach anything or enqueue GenerateQuoteTextJob when the download fails" do
    testimonial = make_tiktok_testimonial
