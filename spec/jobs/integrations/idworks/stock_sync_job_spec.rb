...
RSpec.describe Integrations::Idworks::StockSyncJob do
  ...
  it "calls StockSyncService for the given integration" do
    expect(Integrations::Idworks::StockSyncService).to receive(:call).with(integration)

    described_class.new.perform(integration.id)
  end

  it "does nothing when the integration no longer exists" do
    expect(Integrations::Idworks::StockSyncService).not_to receive(:call)
