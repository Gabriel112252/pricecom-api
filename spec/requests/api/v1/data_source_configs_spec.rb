...
RSpec.describe "Data Source Configs", type: :request do
  ...
  describe "GET /api/v1/data_source_configs" do
    it "returns all 6 data types even with no config rows yet" do
      get "/api/v1/data_source_configs", headers: auth_headers(operador)

      expect(response).to have_http_status(:ok)
      body = JSON.parse(response.body)
      expect(body.map { |c| c["data_type"] }).to contain_exactly(
        "cost", "freight", "tax", "payment_reconciliation", "stock", "order_reconciliation"
      )
