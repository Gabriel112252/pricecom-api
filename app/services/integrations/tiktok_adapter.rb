module Integrations
  ...
  #
  # ⚠️ UNVERIFIED: partner.tiktokshop.com/docv2 is a JS-rendered SPA that
  # couldn't be fetched/read in this environment, so — unlike the Yampi and
  # Shopify adapters — this implementation is NOT grounded against a
  # current, confirmed copy of the docs. It follows the general shape
  # widely documented for TikTok Shop Partner API v2 (signed requests,
  # envelope response with `code`/`message`/`data`, cursor pagination) from
  # training knowledge, which may be stale or subtly wrong. Treat this
  # adapter as a structural placeholder to be corrected against the real
  # docs (or a sandbox account) before it ever runs against production.
  class TiktokAdapter < BaseChannelAdapter
    ...
    def post(path, body)
      ...
      params = { app_key: credentials[:app_key], timestamp: timestamp, access_token: credentials[:access_token] }
      params[:sign] = sign(path, params)

      response = connection(BASE_URL).post(path, body) do |req|
        req.params = params
      end
    ...
    # HMAC-SHA256 over the sorted query params (excluding `sign`) and the
    # request path, keyed by app_secret — the general pattern TikTok
    # Shop's API is documented (elsewhere) to use. Unverified — see class
    # comment.
    def sign(path, params)
      base = params.except(:sign).sort.map { |k, v| "#{k}#{v}" }.join
      signable = "#{credentials[:app_secret]}#{path}#{base}#{credentials[:app_secret]}"
