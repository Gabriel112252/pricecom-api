  ...
  module Shopee
    ...
    class OrdersPollingService
      ...
      def initialize(channel_credential, trigger: "scheduled")
        ...
        @trigger = trigger
        @integration = tenant.integrations.active.find_by(provider: "shopee")
      ...
      def call
        ...
        fetch_and_process_windows
        enqueue_pending_escrow_sync if created_count.positive? || updated_count.positive?
      ...
      def fetch_and_process_windows
        window_slices.each do |slice_from, slice_to|
          fetch_and_process_slice(slice_from, slice_to)
        end
      end
      ...
      def window_slices
        ...
        while from < cursor_to
          to = [ from + WINDOW_SLICE, cursor_to ].min
          slices << [ from, to ]
          from = to
        end
      ...
      def fetch_and_process_slice(slice_from, slice_to)
        cursor = nil

        loop do
          page = adapter.fetch_orders_page(
            time_range_field: time_range_field,
            time_from: slice_from.to_i,
            time_to: slice_to.to_i,
            cursor: cursor,
            page_size: PAGE_SIZE
          )

          @pages_fetched += 1
          entries = page["order_list"] || []
      ...
      def process_order_sns(order_sns)
        ...
        order_sns.each do |sn|
          if seen_external_ids.include?(sn) || fresh_sns.include?(sn)
            record_ignored(sn, "duplicado na mesma execução")
          else
            fresh_sns << sn
          end
        end

        fresh_sns.each_slice(DETAIL_BATCH_SIZE) do |batch|
          details = adapter.fetch_order_details(batch)
          details_by_sn = details.index_by { |d| d["order_sn"].to_s }

          batch.each do |sn|
            seen_external_ids << sn
            detail = details_by_sn[sn]

            if detail.nil?
              record_ignored(sn, "sem detalhe retornado pelo get_order_detail")
              next
            end
      ...
      def process_order(raw_order)
        external_id = raw_order["order_sn"].to_s
        @api_records_received += 1
        ...
        @error_count += 1
        item_errors << { external_id: raw_order["order_sn"]&.to_s, message: e.message }
        record_processed(raw_order["order_sn"]&.to_s, "error")
      end
      ...
      def enqueue_pending_escrow_sync
        ...
        Rails.logger.error("[Integrations::Shopee::OrdersPollingService] escrow enqueue failed: #{e.message}")
      end
      ...
      def base_metadata
        {
          ...
          cursor_source_field: time_range_field,
