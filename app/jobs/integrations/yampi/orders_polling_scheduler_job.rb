  ...
  module Yampi
    class OrdersPollingSchedulerJob < ApplicationJob
      ...
      def polling_locked?(channel_credential)
        Integrations::Yampi::PollingLock.new(channel_credential).locked?
      rescue => e
        Rails.logger.warn("[Integrations::Yampi::OrdersPollingSchedulerJob] lock check failed for channel_credential_id=#{channel_credential.id}: #{e.message}")
        false
      end
    end
  end
