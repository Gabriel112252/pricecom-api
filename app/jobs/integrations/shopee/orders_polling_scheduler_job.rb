  ...
  module Shopee
    class OrdersPollingSchedulerJob < ApplicationJob
      ...
      def perform
        ChannelCredential
          .active
          .where(channel: "shopee", polling_enabled: true)
      ...
      def polling_locked?(channel_credential)
        ...
        Rails.logger.warn("[Integrations::Shopee::OrdersPollingSchedulerJob] lock check failed for channel_credential_id=#{channel_credential.id}: #{e.message}")
        false
      end
    end
  end
